...
do_snapshots() { # properties, flags, snapname, oldglob, [targets...]
	...
	for ii in $TARGETS; do
		...
		if echo "$SNAPSHOTS_OLD" | grep -q "$ii@$NAME"; then
		...
		if [ -n "$opt_do_snapshots" ] && [ "$size_check_skip" -eq 0 ] && [ "$name_collision_skip" -eq 0 ]; then
...
for ii in $CANDIDATES; do
	...
	for jj in "$@"; do
		...
		if [ "$jj" = '//' ] || [ "$jj" = "$ii" ]; then
			IN_ARGS=$((IN_ARGS + 1))
		elif [ -n "$opt_recursive" ] && [ "$iii" != "${iii#$jjj}" ]; then
			IN_ARGS=$((IN_ARGS + 1))
		fi
	done
